...
from .agent import DemoAgent, builder_genesis_txns
...
class FoundationStewardAgent(DemoAgent):
    def __init__(self, http_port: int, admin_port: int, **kwargs):
        super().__init__("Steward Agent", http_port, admin_port, prefix="Steward", **kwargs)
...
async def main():
    ...
    genesis = await builder_genesis_txns()
    if not genesis:
    ...
    try:
        ...
        agent = FoundationStewardAgent(start_port, start_port + 1, genesis_data=genesis)
        await agent.listen_webhooks(start_port + 2)

        # We want to use a peer did? await agent.register_did()
        ...
        log_status("#9 Input Sovrin Foundation Agent invitation details")
        # TODO: can we hardcode this invitation to the Foundation Agent?
